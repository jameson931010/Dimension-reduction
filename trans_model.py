...
class EMG128CAE(nn.Module):
    ...
    def __init__(self, num_pooling: int = 3, num_filter: int = 4, num_conv: int = 2):
        ...
        self.trans_encoder = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(nhead=self.HEAD_NUM, d_model=self.FILTER_NUM, dim_feedforward=2*self.FILTER_NUM, dropout=self.DROP_OUT_RATE, batch_first=True),
            num_layers=self.TRANS_NUM
        )
        ...
        # Transformer
        self.trans_decoder = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(nhead=self.HEAD_NUM, d_model=self.FILTER_NUM, dim_feedforward=2*self.FILTER_NUM, dropout=self.DROP_OUT_RATE, batch_first=True),
            num_layers=self.TRANS_NUM
        )
    ...
    def encode(self, h: torch.Tensor) -> torch.Tensor:
        ...
        h_seq = self.trans_encoder(h_seq)
        h = h_seq.permute(0, 2, 1).view(b, c, he, w)
    ...
    def decode(self, h: torch.Tensor) -> torch.Tensor:
        ...
        b, c, he, w = h.shape
        h_seq = h.flatten(2).permute(0, 2, 1)
        h_seq = self.trans_decoder(h_seq)
        h = h_seq.permute(0, 2, 1).view(b, c, he, w)
